...
{
    public partial class Frm_StockAddedMoneyReport : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("هل انتا متاكد من مسح البيانات", "تاكيد", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                if (rbtnAllStock.Checked)
                {
                    db.executeData("delete from Stock_Insert where Convert(date,Date ,105 ) between '" + date1 + "' and '" + date2 + "'", "تم مسح البيانات بنجاح", "");
                }
                else
                {
                    if (cbxStock.Items.Count > 0)
                    {
                        db.executeData("delete from Stock_Insert where [Stock_Insert].Stock_ID = " + cbxStock.SelectedValue + " and Convert(date,Date ,105 ) between '" + date1 + "' and '" + date2 + "'", "تم مسح البيانات بنجاح", "");
                    }
