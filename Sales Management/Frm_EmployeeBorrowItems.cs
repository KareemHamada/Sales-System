...
{
    public partial class Frm_EmployeeBorrowItems : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("من فضلك اختر منتجا صحيحا", "تاكيد", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            db.executeData("insert into Employee_SalaryMinus (Emp_ID,Date ,Price,Pay) values (" + cbxEmployee.SelectedValue + ",N'" + d + "' ," + totalPrice + " ,'NO')", "", "");

            db.executeData("update Products set Qty=Qty - " + NudQty.Value + " where Pro_ID=" + cbxItems.SelectedValue + "", "", "");

            db.executeData("insert into Employee_BorrowItems values (" + txtID.Text + " , " + cbxItems.SelectedValue + " , " + cbxEmployee.SelectedValue + " , '" + d + "' , " + NudQty.Value + ") ", "تمت اضافة العملية بنجاح", "");
            AutoNumber();
        }
